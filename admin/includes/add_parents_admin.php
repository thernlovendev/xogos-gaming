...
if(isset($_POST['add_parent'])) {
  ...
  if(!empty($username) && !empty($firstname) && !empty($lastname) && !empty($email) && !empty($password) && !empty($repeat_password) ) {
    ...
    $query  = "INSERT INTO users(firstname, lastname, email, username, password, address, city, state, zip, parent_id, user_role) ";
    $query .= "VALUES('{$firstname}', '{$lastname}', '{$email}', '{$username}', '{$password}', '{$address}', '{$city}', '{$state}', '{$zip}', RAND()*(999-1)+5, 'parent' ) ";
